...
export interface CardTiltOptions {
  ...
  /** Glow color (CSS color). Default '#ffffff'. */
  glowColor?: string
  /** Perspective depth. Default 800. */
  perspective?: number
}
 ...
 */
export function useCardTilt(el: Ref<HTMLElement | null>, options: CardTiltOptions = {}) {
  const {
    maxTilt = 8,
    scale = 1.02,
    glowIntensity = 0.3,
 ...
 */
export function useCardTiltAll(
  ...
  onMounted(() => {
    ...
    // Skip if user prefers reduced motion
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return
    container.value.querySelectorAll<HTMLElement>(selector).forEach(card => {
      const {
        maxTilt = 8,
        scale = 1.02,
        perspective = 800,

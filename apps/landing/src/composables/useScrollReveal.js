...
export function useScrollPin(panelCount, options) {
    const panelScrollLength = options?.panelScrollLength ?? 600;
    const containerRef = ref(null);
    const activePanelIndex = ref(0);
    const pinnedStyle = { position: 'sticky', top: '0px' };
    function onScroll() {
        ...
        if (scrolled < 0 || scrolled > totalRange)
            return;
        ...
        mutations.forEach((m) => {
            m.addedNodes.forEach((node) => {
                if (node instanceof Element) {
                    if (node.hasAttribute('data-animate-stagger'))
...
export function useScrollBg(from, to) {
    const bgRef = ref(null);
    const currentBg = ref(from);

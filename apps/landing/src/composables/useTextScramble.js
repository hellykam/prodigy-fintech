...
function runScramble(target, el, opts) {
    ...
    const loop = () => {
        el.textContent = target
            ...
            if (i < (iteration / totalFrames) * target.length)
                return char;
 ...
 */
export function useTextScramble(el, options = {}) {
    const opts = {
        trigger: 'visible',
        duration: 900,
        style: 'tech',
 ...
 */
export function useHoverScramble(el, style = 'tech') {
    const opts = {
        trigger: 'mount', duration: 600, style, threshold: 0.3, delay: 0,
    };
    const scramble = (override) => {
        if (!el.value)
            return;

...
export function useSplitReveal(el, options = {}) {
    const { type = 'words', stagger = 0.06, y = 28, duration = 0.75, ease = 'power3.out', start = 'top 88%', delay = 0, } = options;
    let ctx = null;
    ...
    onMounted(() => {
        ...
        if (type === 'words') {
            element.innerHTML = original
                .split(' ')
                .filter(w => w.length)
                .map(w => `<span class="${WORD_CLASS}" style="display:inline-block;overflow:hidden;vertical-align:bottom;">`
        ...
        ctx = gsap.context(() => {
            const targets = type === 'words' ? element.querySelectorAll(`.${WORD_CLASS}-i`) :
 ...
 */
export function useClipReveal(el, options = {}) {
    ...
    onMounted(() => {
        ...
        ctx = gsap.context(() => {
            gsap.fromTo(element, { clipPath: fromClip, opacity: 1 }, {
                clipPath: toClip,
                duration,
                ease: 'power4.out',
                delay,
 ...
 */
export function useGradientSweep(el, options = {}) {
